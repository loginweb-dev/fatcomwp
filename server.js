...
app.get('/', (req, res) => {
    ...
    if (req.query.type == 'text') {
        ...
        const base64Image = Buffer.from(req.query.message, 'binary').toString('base64');
        const media = new MessageMedia('image/png', base64Image);
        client.sendMessage(chatId, media).then((response) => {
            if (response.id.fromMe) {
                console.log("galeery fue enviado!");
            }
        })
    }
  });
...
client.on("qr", (qr) => {
    ...
    console.log('Nuevo QR, recuerde que se genera cada minut.')
});
client.on('ready', () => {
